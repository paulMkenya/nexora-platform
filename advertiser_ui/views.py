...
from offer.models import (
    ACTIVE_STATUS, Offer, PAUSED_STATUS, offer_statuses,
)
...
#
# The form field parsing / targeting / payout logic is shared with the
# brand-admin operator console; it lives in ``offer.offer_form`` so both
# surfaces stay byte-for-byte identical. These views own only the advertiser +
...
def offer_create(request):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            return render(request, 'advertiser_ui/offer_form.html', offer_form_ctx(request))

        offer = create_offer_from_post(
            request, advertiser=advertiser, brand=getattr(request, 'brand', None))
...
def offer_edit(request, offer_id):
    ...
    if request.method == 'POST':
        ...
        if errors:
            for err in errors:
                messages.error(request, err)
            return render(request, 'advertiser_ui/offer_form.html', offer_form_ctx(request, offer))
